...
namespace CONSTVAL
{
    ...
    const int SONG_LIST_TEXT_VERTICAL_PADDING = 5;
}
...
struct ApperanceSettingData
{
    ...
    bool album_cover_as_background{ true };     //将专辑封面作为背景
    ...
    bool show_notify_icon{ false };             //是否显示通知区图标
    ...
    bool random_background_image{ false };   //随机使用skins/background目录中的背景图片
...
struct GeneralSettingData
{
    ...
    bool portable_mode{ true };                 //强制便携模式，程序数据保存到exe目录下的AppData目录
};
