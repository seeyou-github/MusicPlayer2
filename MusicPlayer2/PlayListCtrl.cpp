...
void CPlayListCtrl::CalculateColumeWidth(vector<int>& width)
{
    ...
    width[0] = theApp.DPI((std::max)(40, 40 + (theApp.m_app_setting_data.song_list_font_size - 16) * 3));
    width[2] = theApp.DPI(70 + (theApp.m_app_setting_data.song_list_font_size - 16) * 4);
    CRect rect;
    GetWindowRect(rect);
...
void CPlayListCtrl::PreSubclassWindow()
{
    ...
    SetRowHeight(theApp.DPI(playlist_item_height));
    SetFont(&theApp.m_font_set.GetFontBySize(theApp.m_app_setting_data.song_list_font_size).GetFont());
    ...
    case CDDS_ITEMPREPAINT:			//如果为画ITEM之前就要进行颜色的改变
        if (IsWindowEnabled())
        {
            ...
            else
            {
                ...
                {
                    lplvdr->clrText = theApp.m_app_setting_data.song_list_custom_text_color ? theApp.m_app_setting_data.song_list_text_color : CColorConvert::m_gray_color.dark3;
