...
protected:
	...
	bool m_save_to_song_folder{ false };		//是否保存到歌曲所在目录
    ...
    bool DownloadSelectedLyricToPreview();
    bool PrepareLyricForDisplayAndSave(const CLyricDownloadCommon::ItemInfo& item, wstring& result);
    bool IsLyricWriteSupported() const;
    void LoadExistingLyricPreview();
    void SetOnlineLyricPreview(const wstring& lyric);
    void UpdateSaveButtonState();
    wstring GetLyricCacheKey(const wstring& song_id) const;
...
private:
    ...
    std::map<wstring, wstring> m_lyric_preview_cache;
...
public:
    ...
    afx_msg void OnBnClickedLyricDlQqmusicRadio();
    afx_msg void OnBnClickedLyricDlNeteaseRadio();
};
