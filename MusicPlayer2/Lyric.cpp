...
namespace
{
    ...
    bool IsForeignLyricChar(wchar_t ch)
    {
        return (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')
            || (ch >= 0x3040 && ch <= 0x30ff) || (ch >= 0xac00 && ch <= 0xd7af);
    }

    bool IsInlineTranslationSeparator(wchar_t ch)
    {
        return ch == L'/' || ch == L' ' || ch == L'\t' || ch == 0x3000 || ch == 0x00a0 || (ch >= 0x2000 && ch <= 0x200b);
    }

    void TrimInlineTranslationPart(wstring& str)
    {
        while (!str.empty() && IsInlineTranslationSeparator(str.front()))
            str.erase(str.begin());
        while (!str.empty() && IsInlineTranslationSeparator(str.back()))
            str.pop_back();
        CCommon::StringNormalize(str);
    }

    bool HasChineseText(const wstring& str)
    {
        int count{};
        for (wchar_t ch : str)
        {
            if (IsChineseChar(ch) && ++count >= 2)
                return true;
        }
        return false;
    }

    bool HasForeignLyricText(const wstring& str)
    {
        for (wchar_t ch : str)
        {
            if (IsForeignLyricChar(ch))
                return true;
        }
        return false;
    }

    bool TryBuildInlineTranslation(const wstring& text, size_t split_pos, size_t translation_pos, wstring& lyric, wstring& translation)
    {
        if (split_pos == wstring::npos || translation_pos == wstring::npos || split_pos == 0 || translation_pos >= text.size())
            return false;

        lyric = text.substr(0, split_pos);
        translation = text.substr(translation_pos);
        TrimInlineTranslationPart(lyric);
        TrimInlineTranslationPart(translation);
        return !lyric.empty() && !translation.empty() && HasForeignLyricText(lyric) && HasChineseText(translation);
    }

    bool TrySplitInlineTranslation(const wstring& text, wstring& lyric, wstring& translation)
    {
        for (size_t i{}; i < text.size(); ++i)
        {
            if (text[i] != L'/')
                continue;
            size_t translation_pos = i + 1;
            while (translation_pos < text.size() && IsInlineTranslationSeparator(text[translation_pos]))
                ++translation_pos;
            if (TryBuildInlineTranslation(text, i, translation_pos, lyric, translation))
                return true;
        }

        for (size_t i{}; i < text.size(); ++i)
        {
            if (!IsInlineTranslationSeparator(text[i]))
                continue;
            size_t translation_pos = i + 1;
            while (translation_pos < text.size() && IsInlineTranslationSeparator(text[translation_pos]))
                ++translation_pos;
            if (translation_pos < text.size() && IsChineseChar(text[translation_pos])
                && TryBuildInlineTranslation(text, i, translation_pos, lyric, translation))
                return true;
        }
...
void CLyrics::AutoSplitInlineTranslations()
{
    ...
    vector<InlineTranslation> detected(m_lyrics.size());
    int non_empty_count{};
    int matched_count{};
    ...
    {
        const Lyric& lyric{ m_lyrics[i] };
        if (lyric.text.empty())
            continue;

        ++non_empty_count;
        if (lyric.translate.empty() && lyric.split.empty() && lyric.word_time.empty()
            && TrySplitInlineTranslation(lyric.text, detected[i].text, detected[i].translate))
        {
            detected[i].matched = true;
            ++matched_count;
            ++consecutive_count;
    ...
    if (matched_count < 4 || (max_consecutive_count < 4 && matched_count * 100 < max(non_empty_count, 1) * 30))
        return;

    for (size_t i{}; i < m_lyrics.size(); ++i)
    {
        if (!detected[i].matched)
            continue;
        m_lyrics[i].text = detected[i].text;
        m_lyrics[i].translate = detected[i].translate;
    }
    m_translate = true;
