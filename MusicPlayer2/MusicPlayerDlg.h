...
protected:
    ...
    vector<wstring> m_background_images;       //skins/background目录中的背景图片完整路径
    wstring m_random_background_image;         //本次启动随机选中的背景图片
...
private:
    ...
    void EnsurePlaylistControlsInitialized();
    void SetAlwaysOnTop();
...
protected:
    ...
    void InitBackgroundImages();        //初始化背景图片列表
    void ApplyBackgroundImageSettings(bool reload_background = true);
    ...
    std::wstring GetBackgroundImageDir() const;
