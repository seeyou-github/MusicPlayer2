...
public:
    ...
    CRect DrawProgressBar(CRect rect, bool play_time_both_side = false, int bar_height = 0, const COLORREF* progress_back_color = nullptr, const COLORREF* progress_color = nullptr, const COLORREF* time_color = nullptr);               //绘制进度条（包含时间）。play_time_both_side如果为true，则播放时间显示的进度条的两侧，否则显示在进度条的右侧（返回进度条部分的矩形区域）
    CRect DrawProgess(CRect rect, int bar_height = 0, const COLORREF* progress_back_color = nullptr, const COLORREF* progress_color = nullptr);                   //绘制进度条（返回进度条部分的矩形区域）
    void DrawABRepeat(CRect rect);      //绘制进度条上的AB重复标记
    void DrawTopRightIcons();           //绘制右上角的图标
    void DrawCurrentTime();             //在右上角绘制当前系统时间
    void DrawAlbumCover(CRect rect, int radius = -1);                //绘制专辑封面
    void DrawAlbumCoverWithInfo(CRect rect, int radius = -1);        //绘制专辑封面，并在上面绘制歌曲的标题和艺术家
    void DrawLyrics(CRect rect, CFont* lyric_font, CFont* lyric_tr_font, bool with_background, bool show_song_info = false,
        const COLORREF* text_color = nullptr, const COLORREF* playing_text_color = nullptr);        //绘制歌词 rect：歌曲区域；with_background是否绘制背景；show_song_info:是否总是在没有歌词时显示歌曲信息
    /**
    ...
    void DrawUiIcon(const CRect& rect, IconMgr::IconType icon_type, IconMgr::IconStyle icon_style, int icon_size);
    ...
    void DrawUIButton(const CRect& rect, UIButton& btn, IconMgr::IconType icon_type, bool big_icon = false, const std::wstring& text = std::wstring(), int font_size = 9, bool checked = false, Alignment align = Alignment::LEFT, bool btn_background = false, int icon_size = 0);
    // 绘制一个UI按钮，以text文本作为图标
    void DrawTextButton(CRect rect, BtnKey btn_type, LPCTSTR text, bool checked = false, const COLORREF* checked_text_color = nullptr, bool checked_background = true, int font_size = 0);
    void DrawTextButton(CRect rect, UIButton& btn, LPCTSTR text, bool checked = false, const COLORREF* checked_text_color = nullptr, bool checked_background = true, int font_size = 0);
