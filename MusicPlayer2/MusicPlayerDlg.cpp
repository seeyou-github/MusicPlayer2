...
void CMusicPlayerDlg::SaveConfig()
{
    ...
    ini.WriteBool(L"config", L"show_notify_icon", theApp.m_app_setting_data.show_notify_icon);
    ...
    ini.WriteBool(L"config", L"random_background_image", theApp.m_app_setting_data.random_background_image);
    ...
    ini.WriteBool(L"general", L"auto_download_lyric_to_audio_file", theApp.m_general_setting_data.auto_download_lyric_to_audio_file);
    ini.WriteBool(L"general", L"auto_download_cover_to_audio_file", theApp.m_general_setting_data.auto_download_cover_to_audio_file);
...
void CMusicPlayerDlg::LoadConfig()
{
    ...
    m_window_width = ini.GetInt(L"config", L"window_width", 1525);
    m_window_height = ini.GetInt(L"config", L"window_hight", 918);
    theApp.m_app_setting_data.window_transparency = ini.GetInt(L"config", L"transparency", 95);
    ...
    theApp.m_nc_setting_data.playlist_size.cx = ini.GetInt(L"config", L"float_playlist_width", 396);
    theApp.m_nc_setting_data.playlist_size.cy = ini.GetInt(L"config", L"float_playlist_height", 597);
    ...
    theApp.m_app_setting_data.show_spectrum = ini.GetBool(L"config", L"show_spectrum", false);
    ...
    theApp.m_lyric_setting_data.desktop_lyric_data.lyric_font.name = ini.GetString(L"desktop_lyric", L"font_name", L"Microsoft YaHei UI");
    ...
    theApp.m_lyric_setting_data.desktop_lyric_data.text_color1 = ini.GetInt(L"desktop_lyric", L"text_color1", 13361120);
    theApp.m_lyric_setting_data.desktop_lyric_data.text_color2 = ini.GetInt(L"desktop_lyric", L"text_color2", 14016991);
    ...
    theApp.m_app_setting_data.ui_refresh_interval = ini.GetInt(L"config", L"ui_refresh_interval", 50);
    ...
    theApp.m_app_setting_data.notify_icon_selected = ini.GetInt(L"config", L"notify_icon_selected", 1);
    theApp.m_app_setting_data.notify_icon_auto_adapt = ini.GetBool(L"config", L"notify_icon_auto_adapt", true);
    theApp.m_app_setting_data.show_notify_icon = ini.GetBool(L"config", L"show_notify_icon", false);
    ...
    theApp.m_app_setting_data.default_background = ini.GetString(L"config", L"default_background", L"");
    theApp.m_app_setting_data.use_desktop_background = ini.GetBool(L"config", L"use_desktop_background", false);
    theApp.m_app_setting_data.random_background_image = ini.GetBool(L"config", L"random_background_image", false);
    ...
    theApp.m_app_setting_data.show_minimode_btn_in_titlebar = ini.GetBool(L"config", L"show_minimode_btn_in_titlebar", false);
    theApp.m_app_setting_data.show_fullscreen_btn_in_titlebar = ini.GetBool(L"config", L"show_fullscreen_btn_in_titlebar", false);
    theApp.m_app_setting_data.show_skin_btn_in_titlebar = ini.GetBool(L"config", L"show_skin_btn_in_titlebar", true);
    theApp.m_app_setting_data.show_settings_btn_in_titlebar = ini.GetBool(L"config", L"show_settings_btn_in_titlebar", true);
    ...
    theApp.m_nc_setting_data.dark_mode_default_transparency = ini.GetInt(L"other", L"dark_mode_default_transparency", 80);
    theApp.m_nc_setting_data.debug_log = ini.GetInt(L"other", L"debug_log", 2);
    theApp.m_nc_setting_data.show_close_main_window_inquery = ini.GetBool(L"other", L"show_close_main_window_inquery", true);

    ini.GetStringList(L"config", L"default_file_type", theApp.m_nc_setting_data.default_file_type, vector<wstring>{L"mp3", L"wma", L"wav", L"flac", L"ogg", L"oga", L"m4a", L"mp4", L"webm", L"cue", L"mp2", L"mp1", L"aif", L"aiff", L"asf"});
    if (!CCommon::IsItemInVector(theApp.m_nc_setting_data.default_file_type, wstring{ L"webm" }))
        theApp.m_nc_setting_data.default_file_type.push_back(L"webm");
    ...
    theApp.m_play_setting_data.stop_when_play_device_changed = ini.GetBool(L"config", L"stop_when_play_device_changed", true);
    theApp.m_play_setting_data.open_single_file_in_folder_mode = ini.GetBool(L"config", L"open_single_file_in_folder_mode", false);
    theApp.m_play_setting_data.output_device = ini.GetString(L"config", L"output_device", theApp.m_str_table.LoadText(L"TXT_OPT_PLAY_DEVICE_NAME_BASS_DEFAULT").c_str());
    ...
    for (int i{}; i < static_cast<int>(m_ui_list.size()); ++i)
    {
        ...
        if (user_ui != nullptr && CCommon::StringCompareNoCase(user_ui->GetXmlPath(), theApp.m_local_dir + L"skins\\my_ui.xml"))
        {
    ...
    CRecentList::listSortMode playlist_sort_mode = static_cast<CRecentList::listSortMode>(ini.GetInt(L"other", L"playlist_sort_mode", 1));
    CRecentList::Instance().SetSortMode(LT_PLAYLIST, playlist_sort_mode);
    CRecentList::listSortMode folder_sort_mode = static_cast<CRecentList::listSortMode>(ini.GetInt(L"other", L"folder_sort_mode", 1));
    ...
    if (theApp.m_media_lib_setting_data.folder_tab_paths.empty() && !theApp.m_media_lib_setting_data.folder_tab_names.empty())
    {
        ...
        theApp.m_media_lib_setting_data.folder_tab_names.clear();
        if (old_names.size() == theApp.m_media_lib_setting_data.media_folders.size())
        {
            for (size_t i{}; i < old_names.size(); ++i)
            {
                if (!old_names[i].empty())
                {
                    theApp.m_media_lib_setting_data.folder_tab_paths.push_back(theApp.m_media_lib_setting_data.media_folders[i]);
                    theApp.m_media_lib_setting_data.folder_tab_names.push_back(old_names[i]);
                }
            }
        }
        else if (old_names.size() + 1 == theApp.m_media_lib_setting_data.media_folders.size()
            && !theApp.m_media_lib_setting_data.media_folders.empty()
            && CCommon::StringCompareNoCase(theApp.m_media_lib_setting_data.media_folders.front(), CCommon::GetSpecialDir(CSIDL_MYMUSIC)))
        {
            for (size_t i{}; i < old_names.size(); ++i)
            {
                if (!old_names[i].empty())
                {
                    theApp.m_media_lib_setting_data.folder_tab_paths.push_back(theApp.m_media_lib_setting_data.media_folders[i + 1]);
                    theApp.m_media_lib_setting_data.folder_tab_names.push_back(old_names[i]);
                }
            }
        }
    }
    if (theApp.m_media_lib_setting_data.folder_tab_names.size() > theApp.m_media_lib_setting_data.folder_tab_paths.size())
        theApp.m_media_lib_setting_data.folder_tab_names.resize(theApp.m_media_lib_setting_data.folder_tab_paths.size());
    ...
    theApp.m_app_setting_data.song_list_font_size = ini.GetInt(L"config", L"song_list_font_size", 16);
...
{
    if (!m_playlist_controls_initialized)
        return;
...
void CMusicPlayerDlg::ShowPlayList(bool highlight_visible)
{
    ...
    if (m_playlist_controls_initialized && theApp.m_ui_data.show_playlist)
    {
...
void CMusicPlayerDlg::SetPlayListColor(bool highlight_visible)
{
    ...
    if (m_playlist_controls_initialized && theApp.m_ui_data.show_playlist)
    {
        m_playlist_list.SetHightItem(current_index);
        //m_playlist_list.SetColor(theApp.m_app_setting_data.theme_color);
        m_playlist_list.Invalidate(FALSE);
        if (highlight_visible && current_index >= 0)
        {
            m_playlist_list.EnsureVisible(current_index, FALSE);
...
{
    if (theApp.m_ui_data.show_playlist)
        EnsurePlaylistControlsInitialized();
...
void CMusicPlayerDlg::EnsurePlaylistControlsInitialized()
{
    if (m_playlist_controls_initialized)
        return;

    int playlist_item_height = CalculateSongListItemHeight(theApp.m_app_setting_data.song_list_font_size);
    int row_height{ theApp.DPI(playlist_item_height) };
    CFont* song_list_font{ &theApp.m_font_set.GetFontBySize(theApp.m_app_setting_data.song_list_font_size).GetFont() };
    m_playlist_list.SetFont(song_list_font);
    ...
    m_playlist_controls_initialized = true;
}
...
void CMusicPlayerDlg::ApplyApperanceSettings(const ApperanceSettingData& apperence_data)
{
    ...
    bool notify_icon_changed{ theApp.m_app_setting_data.notify_icon_selected != apperence_data.notify_icon_selected
                           || theApp.m_app_setting_data.notify_icon_auto_adapt != apperence_data.notify_icon_auto_adapt
                           || theApp.m_app_setting_data.show_notify_icon != apperence_data.show_notify_icon };
    bool default_background_changed{ theApp.m_app_setting_data.default_background != apperence_data.default_background
                                 ...
                                 || theApp.m_app_setting_data.random_background_image != apperence_data.random_background_image };
    ...
    {
        if (theApp.m_app_setting_data.show_notify_icon && theApp.m_app_setting_data.notify_icon_auto_adapt)
        {
    ...
    if (song_list_appearance_changed)
    {
        if (song_list_font_size_changed)
        {
            int playlist_item_height = CalculateSongListItemHeight(apperence_data.song_list_font_size);
            int row_height{ theApp.DPI(playlist_item_height) };
            CFont* song_list_font{ &theApp.m_font_set.GetFontBySize(apperence_data.song_list_font_size).GetFont() };
            if (m_playlist_controls_initialized)
            {
                m_playlist_list.SetFont(song_list_font);
                m_playlist_list.SetRowHeight(row_height);
            ...
            if (IsFloatPlaylistExist())
            {
                m_pFloatPlaylistDlg->GetListCtrl().SetFont(song_list_font);
                m_pFloatPlaylistDlg->GetListCtrl().SetRowHeight(row_height);
                m_pFloatPlaylistDlg->GetListCtrl().AdjustColumnWidth();
            }
            if (m_miniModeDlg.GetSafeHwnd() != NULL)
            {
                m_miniModeDlg.GetPlaylistCtrl().SetFont(song_list_font);
                m_miniModeDlg.GetPlaylistCtrl().SetRowHeight(row_height);
                m_miniModeDlg.GetPlaylistCtrl().AdjustColumnWidth();
            }
            CMyPlayerListCache::UpdateAllTabMetrics(apperence_data.song_list_font_size, playlist_item_height);
        }
        if (m_playlist_controls_initialized)
            m_playlist_list.Invalidate();
        if (IsFloatPlaylistExist())
            m_pFloatPlaylistDlg->GetListCtrl().Invalidate();
        if (m_miniModeDlg.GetSafeHwnd() != NULL)
            m_miniModeDlg.GetPlaylistCtrl().Invalidate();
        m_ui_thread_para.ui_force_refresh = true;
    }
...
void CMusicPlayerDlg::SetMenuState(CMenu* pMenu)
{
    ...
    bool use_album_background{ theApp.m_app_setting_data.album_cover_as_background };
    pMenu->CheckMenuRadioItem(ID_BACKGROUND_USE_ALBUM, ID_BACKGROUND_USE_IMAGE,
        use_album_background ? ID_BACKGROUND_USE_ALBUM : ID_BACKGROUND_USE_IMAGE, MF_BYCOMMAND | MF_CHECKED);
    pMenu->CheckMenuItem(ID_BACKGROUND_RANDOM_IMAGE, MF_BYCOMMAND | (theApp.m_app_setting_data.random_background_image ? MF_CHECKED : MF_UNCHECKED));
    pMenu->EnableMenuItem(ID_BACKGROUND_RANDOM_IMAGE, MF_BYCOMMAND | (!use_album_background && !m_background_images.empty() ? MF_ENABLED : MF_GRAYED));
    pMenu->EnableMenuItem(ID_BACKGROUND_IMAGE_LIST, MF_BYCOMMAND | MF_GRAYED);
    for (size_t i{}; i < m_background_images.size() && i < ID_BACKGROUND_IMAGE_MAX - ID_BACKGROUND_IMAGE_START + 1; ++i)
    {
        UINT id = ID_BACKGROUND_IMAGE_START + static_cast<UINT>(i);
        bool checked{ !theApp.m_app_setting_data.random_background_image && theApp.m_app_setting_data.default_background == m_background_images[i] };
        pMenu->CheckMenuItem(id, MF_BYCOMMAND | (checked ? MF_CHECKED : MF_UNCHECKED));
        pMenu->EnableMenuItem(id, MF_BYCOMMAND | (!use_album_background && !theApp.m_app_setting_data.random_background_image ? MF_ENABLED : MF_GRAYED));
    }
    ...
    pMenu->EnableMenuItem(ID_DOWNLOAD_ALBUM_COVER, MF_BYCOMMAND | (!CPlayer::GetInstance().IsOsuFile() ? MF_ENABLED : MF_GRAYED));
...
{
    vector<MenuMgr::MenuItem> menu_list;
    menu_list.emplace_back(MenuMgr::MenuItem{ ID_BACKGROUND_USE_ALBUM, IconMgr::IconType::IT_Album_Cover, theApp.m_str_table.LoadText(L"TXT_BG_USE_ALBUM") });
    menu_list.emplace_back(MenuMgr::MenuItem{ ID_BACKGROUND_USE_IMAGE, IconMgr::IconType::IT_Background, theApp.m_str_table.LoadText(L"TXT_BG_USE_IMAGE") });
    menu_list.emplace_back(MenuMgr::MenuItem{ ID_BACKGROUND_RANDOM_IMAGE, IconMgr::IconType::IT_NO_ICON, theApp.m_str_table.LoadText(L"TXT_BG_RANDOM_IMAGE") });
    menu_list.emplace_back(MenuMgr::MenuItem{ ID_BACKGROUND_IMAGE_LIST, IconMgr::IconType::IT_NO_ICON, theApp.m_str_table.LoadText(L"TXT_BG_IMAGE_LIST") });
    for (size_t i{}; i < m_background_images.size() && i < ID_BACKGROUND_IMAGE_MAX - ID_BACKGROUND_IMAGE_START + 1; ++i)
    {
        wstring file_name{ CFilePathHelper(m_background_images[i]).GetFileName() };
        menu_list.emplace_back(MenuMgr::MenuItem{ ID_BACKGROUND_IMAGE_START + static_cast<UINT>(i), IconMgr::IconType::IT_NO_ICON, L"  " + file_name });
    }
    menu_list.emplace_back(MenuMgr::MenuItem{});

    size_t ui_menu_count{};
    for (size_t i{}; i < m_ui_list.size(); ++i)
    {
        if (ui_menu_count >= SELECT_UI_MAX_SIZE)
            break;
...
std::wstring CMusicPlayerDlg::GetBackgroundImageDir() const
{
    return theApp.m_local_dir + L"skins\\background\\";
}

void CMusicPlayerDlg::InitBackgroundImages()
{
    m_background_images.clear();
    vector<wstring> jpg_files;
    vector<wstring> png_files;
    const wstring image_dir{ GetBackgroundImageDir() };
    CCommon::GetFiles(image_dir + L"*.jpg", jpg_files);
    CCommon::GetFiles(image_dir + L"*.jpeg", png_files);
    jpg_files.insert(jpg_files.end(), png_files.begin(), png_files.end());
    png_files.clear();
    CCommon::GetFiles(image_dir + L"*.png", png_files);
    jpg_files.insert(jpg_files.end(), png_files.begin(), png_files.end());

    std::sort(jpg_files.begin(), jpg_files.end(), [](const wstring& a, const wstring& b)
        {
            return CCommon::StringCompareInLocalLanguage(a, b, true) < 0;
        });
    for (const auto& file : jpg_files)
        m_background_images.push_back(image_dir + file);

    ApplyBackgroundImageSettings(false);
}

void CMusicPlayerDlg::ApplyBackgroundImageSettings(bool reload_background)
{
    theApp.m_app_setting_data.use_desktop_background = false;
    if (m_background_images.empty())
    {
        theApp.m_app_setting_data.default_background.clear();
        m_random_background_image.clear();
        if (reload_background)
            LoadDefaultBackground();
        return;
    }

    auto iter = std::find(m_background_images.begin(), m_background_images.end(), theApp.m_app_setting_data.default_background);
    if (iter == m_background_images.end())
        theApp.m_app_setting_data.default_background = m_background_images.front();

    if (theApp.m_app_setting_data.random_background_image || theApp.m_app_setting_data.album_cover_as_background)
    {
        std::random_device rd;
        std::mt19937 generator(rd());
        std::uniform_int_distribution<size_t> distribution(0, m_background_images.size() - 1);
        m_random_background_image = m_background_images[distribution(generator)];
    }
    else
    {
        m_random_background_image.clear();
...
void CMusicPlayerDlg::LoadDefaultBackground()
{
    ...
    wstring background_img;
    if (theApp.m_app_setting_data.random_background_image || theApp.m_app_setting_data.album_cover_as_background)
        background_img = m_random_background_image;
    else
        background_img = theApp.m_app_setting_data.default_background;

    if (background_img.empty() || !CCommon::FileExist(background_img))
    {
        if (!m_background_images.empty())
        {
            background_img = m_background_images.front();
            if (!theApp.m_app_setting_data.random_background_image && !theApp.m_app_setting_data.album_cover_as_background)
                theApp.m_app_setting_data.default_background = background_img;
        }
    }

    if (!background_img.empty())
        theApp.m_ui_data.default_background.Load(background_img.c_str());
...
BOOL CMusicPlayerDlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
    ...
    if (command == ID_BACKGROUND_USE_ALBUM)
    {
        theApp.m_app_setting_data.album_cover_as_background = true;
        CPlayer::GetInstance().AlbumCoverGaussBlur();
        LoadDefaultBackground();
        DrawInfo(true);
        SaveConfig();
    }
    else if (command == ID_BACKGROUND_USE_IMAGE)
    {
        theApp.m_app_setting_data.album_cover_as_background = false;
        ApplyBackgroundImageSettings();
        DrawInfo(true);
        SaveConfig();
    }
    else if (command == ID_BACKGROUND_RANDOM_IMAGE)
    {
        if (!theApp.m_app_setting_data.album_cover_as_background && !m_background_images.empty())
        {
            theApp.m_app_setting_data.random_background_image = !theApp.m_app_setting_data.random_background_image;
            ApplyBackgroundImageSettings();
            DrawInfo(true);
            SaveConfig();
        }
    }
    else if (command >= ID_BACKGROUND_IMAGE_START && command <= ID_BACKGROUND_IMAGE_MAX)
    {
        size_t image_index = command - ID_BACKGROUND_IMAGE_START;
        if (!theApp.m_app_setting_data.album_cover_as_background && image_index < m_background_images.size())
        {
            theApp.m_app_setting_data.random_background_image = false;
            theApp.m_app_setting_data.default_background = m_background_images[image_index];
            ApplyBackgroundImageSettings();
            DrawInfo(true);
            SaveConfig();
        }
...
void CMusicPlayerDlg::OnSplitterChanged(CRect splitter_rect)
{
    ...
    if (pThis != nullptr)
    {
        ...
        if (pThis->m_playlist_controls_initialized)
...
UINT CMusicPlayerDlg::DownloadLyricAndCoverThreadFunc(LPVOID lpParam)
{
    ...
    if (download_cover && !is_osu)  // osu文件不进行自动下载专辑封面
    {
        ...
        bool saved_cover_to_audio_file{};
        if (theApp.m_general_setting_data.auto_download_cover_to_audio_file)
        {
            vector<unsigned char> cover_data;
            if (DownloadUrlToMemory(cover_url, cover_data))
            {
                CAudioTag audio_tag(song_info_ori.file_path);
                wstring cover_ext = CFilePathHelper(cover_url).GetFileExtension();
                if (audio_tag.WriteAlbumCover(BytesToString(cover_data), cover_ext.empty() ? L"jpg" : cover_ext))
                {
                    saved_cover_to_audio_file = true;
        ...
        if (!saved_cover_to_audio_file)
        {
            ...
            if (theApp.m_general_setting_data.auto_download_cover_to_audio_file)
                saved_to_album_cover_folder = true;
    ...
    if (download_lyric)
    {
        ...
        if (theApp.m_general_setting_data.auto_download_lyric_to_audio_file)
        {
            CAudioTag audio_tag(song_info_ori.file_path);
...
void CMusicPlayerDlg::OnAlbumCoverSaveAs()
{
    ...
    if (IDOK == fileDlg.DoModal())
    {
        ...
        if (CPlayer::GetInstance().IsInnerCover())
        {
            if (!CPlayer::GetInstance().GetAlbumCover().IsNull())
                CPlayer::GetInstance().GetAlbumCover().Save(dest_file);
        }
        else
        {
...
afx_msg LRESULT CMusicPlayerDlg::OnMusicStreamOpened(WPARAM wParam, LPARAM lParam)
{
    ...
    CPlayer::GetInstance().AlbumCoverGaussBlur();
    if (theApp.m_app_setting_data.album_cover_as_background && !CPlayer::GetInstance().AlbumCoverExist())
        ApplyBackgroundImageSettings();
    //自动下载专辑封面
