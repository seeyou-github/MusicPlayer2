...
void CUIDrawer::SetLyricColors(const COLORREF* text_color, const COLORREF* playing_text_color)
{
    m_custom_lyric_text_color = (text_color != nullptr);
    m_custom_lyric_playing_text_color = (playing_text_color != nullptr);
    if (text_color != nullptr)
        m_lyric_text_color = *text_color;
    if (playing_text_color != nullptr)
        m_lyric_playing_text_color = *playing_text_color;
}

COLORREF CUIDrawer::LyricTextColor() const
{
    return m_custom_lyric_text_color ? m_lyric_text_color : m_colors.color_text_2;
}

COLORREF CUIDrawer::LyricPlayingTextColor() const
{
    return m_custom_lyric_playing_text_color ? m_lyric_playing_text_color : m_colors.color_text;
}
...
void CUIDrawer::DrawLyricTextMultiLine(CRect lyric_area, Alignment align, bool show_song_info)
{
    ...
    if (CPlayer::GetInstance().IsPlaylistEmpty())   //当前播放为空时在歌词区域显示播放提示
    {
        ...
        DrawWindowText(lyric_area, no_track_tip_str.c_str(), LyricTextColor(), Alignment::LEFT, false, true);
        SetFont(font);
    }
    else if (CPlayerUIHelper::IsMidiLyric())
    {
        wstring current_lyric{ CPlayer::GetInstance().GetMidiLyric() };
        DrawWindowText(lyric_area, current_lyric.c_str(), LyricPlayingTextColor(), Alignment::CENTER, false, true);
    }
    else if (CPlayer::GetInstance().m_Lyrics.IsEmpty())
    {
        ...
        if (theApp.m_lyric_setting_data.show_song_info_if_lyric_not_exist || show_song_info)
        {
            ...
            DrawWindowText(lyric_area, song_info_str, LyricPlayingTextColor(), align, false, true);
        }
        //显示“当前歌曲没有歌词”
        else
        {
            static const wstring& no_lyric_info = theApp.m_str_table.LoadText(L"UI_LYRIC_NONE");
            DrawWindowText(lyric_area, no_lyric_info.c_str(), LyricTextColor(), Alignment::CENTER);
        }
    }
    else
    {
        ...
        for (int i{ -1 }; i < static_cast<int>(rects.size()) - 1; i++)
        {
            ...
            if (!(rects[i + 1] & lyric_area).IsRectEmpty())		//只有当一句歌词的矩形区域和歌词区域的矩形有交集时，才绘制歌词
            {
                ...
                if (i == lyric_index && progress < 1000)		//绘制正在播放的歌词（处于逐渐过度到高亮过程中的歌词）
                {
                    ...
                    COLORREF text_color = CColorConvert::GetGradientColor(LyricTextColor(), LyricPlayingTextColor(), fade_percent);
                    //绘制歌词文本
                    SetFont(m_lyric_font);
                    if (theApp.m_lyric_setting_data.lyric_karaoke_disp)
                        DrawWindowText(rect_text, lyric_i.text.c_str(), LyricPlayingTextColor(), LyricTextColor(), progress, align, true);
                    else
                    ...
                    if (!lyric_i.translate.empty() && theApp.m_lyric_setting_data.show_translate)
                    {
                        ...
                        DrawWindowText(rect_translate, lyric_i.translate.c_str(), LyricTextColor(), align, true);
                    }
                }
                else		//绘制非正在播放的歌词
                {
                    ...
                    if (i == lyric_index - 1 || (i == lyric_index && progress == 1000))         // 绘制正在取消高亮的歌词（这里实现一句歌词颜色从高亮缓慢变化到非高亮效果），逐字歌词最后一句在此处取消高亮
                    {
                        ...
                        text_color = CColorConvert::GetGradientColor(LyricPlayingTextColor(), LyricTextColor(), fade_percent);
                    }
                    else
                    {
                        text_color = LyricTextColor();
                    }
                    //绘制歌词文本
...
void CUIDrawer::DrawLyricTextSingleLine(CRect rect, int& flag, bool double_line, Alignment align, bool show_song_info)
{
    ...
    {
        wstring current_lyric{ CPlayer::GetInstance().GetMidiLyric() };
        DrawWindowText(rect, current_lyric.c_str(), LyricPlayingTextColor(), Alignment::CENTER, false, true);
    }
    else if (CPlayer::GetInstance().m_Lyrics.IsEmpty())
    {
        //没有歌词时显示歌曲信息
        if (theApp.m_lyric_setting_data.show_song_info_if_lyric_not_exist || show_song_info)
        {
            ...
            DrawScrollText(rect, song_info_str.c_str(), LyricPlayingTextColor(), CPlayerUIHelper::GetScrollTextPixel(), theApp.m_lyric_setting_data.lyric_align != Alignment::LEFT, lyric_scroll_info);
        }
        //显示“当前歌曲没有歌词”
        else
        {
            static const wstring& no_lyric_info = theApp.m_str_table.LoadText(L"UI_LYRIC_NONE");
            DrawWindowText(rect, no_lyric_info.c_str(), LyricTextColor(), Alignment::CENTER);
        }
    }
    else
    {
        ...
        if (no_lyric && show_song_info)
        {
            ...
            static CDrawCommon::ScrollInfo lyric_scroll_info;
            DrawScrollText(rect, song_info_str, LyricPlayingTextColor(), CPlayerUIHelper::GetScrollTextPixel(), theApp.m_lyric_setting_data.lyric_align != Alignment::LEFT, lyric_scroll_info);
        }
        else
        {
            ...
            else
            {
                ...
                if (theApp.m_lyric_setting_data.show_translate && !current_lyric.translate.empty() && rect.Height() > static_cast<int>(GetLyricTextHeight() * 1.73))
                {
                    ...
                    SetFont(m_lyric_tr_font);
                    DrawWindowText(translate_rect, current_lyric.translate.c_str(), LyricTextColor(), align, true);
                }
                // 绘制单行歌词
                SetFont(m_lyric_font);
                if (theApp.m_lyric_setting_data.lyric_karaoke_disp)
                    DrawWindowText(lyric_rect, current_lyric.text.c_str(), LyricPlayingTextColor(), LyricTextColor(), progress, align, true);
                else if (0 < progress && progress < 1000)   // 仅高亮“正在进行”的歌词
                    DrawWindowText(lyric_rect, current_lyric.text.c_str(), LyricPlayingTextColor(), LyricPlayingTextColor(), progress, align, true);
                else
                    DrawWindowText(lyric_rect, current_lyric.text.c_str(), LyricTextColor(), LyricTextColor(), progress, align, true);
            }
        }
    }
...
void CUIDrawer::DrawLyricDoubleLine(CRect rect, LPCTSTR lyric, LPCTSTR next_lyric, Alignment align, int progress, bool switch_flag, int fade_percent)
{
    ...
    if (theApp.m_lyric_setting_data.lyric_karaoke_disp)
    {
        color1 = LyricPlayingTextColor();
        color2 = LyricTextColor();
    }
    else
    {
        color1 = color2 = CColorConvert::GetGradientColor(LyricTextColor(), LyricPlayingTextColor(), fade_percent);
    }
    ...
    if (!switch_flag)
    {
        ...
        DrawWindowText(down_rect, next_lyric, LyricTextColor(), down_align);
    }
    else
    {
        DrawWindowText(up_rect, next_lyric, LyricTextColor(), up_align);
