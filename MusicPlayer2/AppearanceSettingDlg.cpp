...
bool CAppearanceSettingDlg::InitializeControls()
{
    ...
    temp = theApp.m_str_table.LoadText(L"TXT_OPT_APC_SONG_LIST_FONT_SIZE");
    SetDlgItemTextW(IDC_TXT_SONG_LIST_FONT_SIZE_STATIC, temp.c_str());
    temp = theApp.m_str_table.LoadText(L"TXT_OPT_APC_SONG_LIST_TEXT_COLOR");
    SetDlgItemTextW(IDC_TXT_SONG_LIST_TEXT_COLOR_STATIC, temp.c_str());
    ...
    SetDlgControlText(IDC_SHOW_TITLE_BAR_BACKGROUND_CHECK, L"TXT_OPT_APC_SHOW_TITLE_BAR_BACKGROUND");
    ...
    SetDlgControlText(IDC_SHOW_NOTIFY_ICON_CHECK, L"TXT_OPT_APC_SHOW_NOTIFY_ICON");
...
void CAppearanceSettingDlg::SetControlEnable()
{
    ...
    m_album_cover_name_edit.EnableWindow(false);
    m_album_cover_path_edit.EnableWindow(false);
    ...
    EnableDlgCtrl(IDC_SHOW_TITLE_BAR_BACKGROUND_CHECK, !m_data.show_window_frame);
...
void CAppearanceSettingDlg::UpdateSongListTextColorControls()
{
    m_song_list_text_color_theme_chk.SetCheck(!m_data.song_list_custom_text_color);
...
void CAppearanceSettingDlg::GetDataFromUi()
{
    ...
    m_data.song_list_font_size = m_song_list_font_size_sld.GetPos();
    ...
    m_data.show_titlebar_background = (IsDlgButtonChecked(IDC_SHOW_TITLE_BAR_BACKGROUND_CHECK) != 0);
    ...
    m_data.default_album_name = { L"cover" };
    m_data.album_cover_path = L"AppData\\cache\\cover\\";
...
BEGIN_MESSAGE_MAP(CAppearanceSettingDlg, CTabDlg)
    ...
    ON_WM_PAINT()
    ON_BN_CLICKED(IDC_SHOW_NOTIFY_ICON_CHECK, &CAppearanceSettingDlg::OnBnClickedShowNotifyIconCheck)
...
BOOL CAppearanceSettingDlg::OnInitDialog()
{
    ...
    m_data.default_album_name = { L"cover" };
    m_data.album_cover_path = L"AppData\\cache\\cover\\";
    m_album_cover_name_edit.SetWindowTextW(L"cover");
    m_album_cover_name_edit.EnableBrowseButton(FALSE);
    m_album_cover_name_edit.SetReadOnly(TRUE);
    m_album_cover_path_edit.SetWindowTextW(theApp.m_app_setting_data.AbsoluteAlbumCoverPath().c_str());
    m_album_cover_path_edit.EnableBrowseButton(FALSE);
    m_album_cover_path_edit.SetReadOnly(TRUE);
    ...
    m_song_list_font_size_sld.SetRange(15, 30);
    m_song_list_font_size_sld.SetPos(m_data.song_list_font_size);
    ...
    CheckDlgButton(IDC_SHOW_TITLE_BAR_BACKGROUND_CHECK, m_data.show_titlebar_background);
    ...
    m_song_list_font_size_sld.SetMouseWheelEnable(false);
...
void CAppearanceSettingDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
    ...
    if ((pScrollBar->GetDlgCtrlID() == IDC_SONG_LIST_FONT_SIZE_SLIDER))
    {
        m_data.song_list_font_size = m_song_list_font_size_sld.GetPos();
        CCommon::SetNumRange(m_data.song_list_font_size, 15, 30);
        CString str;
...
void CAppearanceSettingDlg::OnBnClickedShowNotifyIconCheck()
{
    m_data.show_notify_icon = (IsDlgButtonChecked(IDC_SHOW_NOTIFY_ICON_CHECK) != 0);
    SetControlEnable();
    InvalidateRect(m_notify_icon_preview, FALSE);
...
void CAppearanceSettingDlg::OnBnClickedSongListTextColorButton()
{
    COLORREF init_color{ m_data.song_list_custom_text_color ? m_data.song_list_text_color : CPlayerUIHelper::GetUIColors(m_data.dark_mode).color_text };
...
void CAppearanceSettingDlg::OnBnClickedSongListTextColorThemeCheck()
{
    m_data.song_list_custom_text_color = (m_song_list_text_color_theme_chk.GetCheck() == 0);
