...
static void DeleteCacheFile(const wchar_t* file_name)
{
    if (file_name != nullptr)
        ::DeleteFile((CCommon::GetTemplatePath() + file_name).c_str());
}

static bool LoadImageFromMemory(const string& image_contents, CImage& image)
{
...
void CPlayer::Create()
{
    ...
    m_defer_open_until_play = !theApp.m_play_setting_data.remember_last_position && !theApp.m_play_setting_data.auto_play_when_start;
...
void CPlayer::IniPlaylistComplate()
{
    ...
    if (!IsPlaylistEmpty())         // 播放列表初始化完成，根据m_index,m_current_position,m_thread_info.play还原播放状态
    {
        ...
        const bool open_current_track{ !m_defer_open_until_play || theApp.m_play_setting_data.remember_last_position || theApp.m_play_setting_data.auto_play_when_start || m_thread_info.play || tmp_find };
...
void CPlayer::MusicControl(Command command, int volume_step)
{
    ...
    if ((command == Command::OPEN || command == Command::PLAY || command == Command::PLAY_PAUSE) && !m_file_opend && m_index < 0 && !IsPlaylistEmpty())
    {
        m_index = 0;
        m_defer_open_until_play = false;
    }
    ...
    case Command::OPEN:
    {
        ...
        if (m_pCore->GetCoreType() == PT_BASS && GetBassHandle() == 0 && CAudioCommon::GetAudioTypeByFileName(cur_song.file_path) == AU_OPUS)
        {
            std::unique_ptr<CFfmpegCore> ffmpeg_core{ new CFfmpegCore() };
            if (ffmpeg_core->IsSucceed())
            {
                m_pCore->UnInitCore();
                delete m_pCore;
                m_pCore = ffmpeg_core.release();
                m_pCore->InitCore();
                m_error_code = 0;
                m_error_state = ES_NO_ERROR;
                theApp.m_play_setting_data.use_mci = false;
                theApp.m_play_setting_data.use_ffmpeg = true;
                m_pCore->Open(cur_song.file_path.c_str());
                GetPlayerCoreError(L"Open");
            }
        }
...
void CPlayer::SaveConfig() const
{
    ...
    ini.WriteString(L"config", L"lyric_path", L"AppData\\cache\\lyrices\\");
    ...
    ini.WriteString(L"config", L"default_album_file_name", L"cover");
    ini.WriteString(L"config", L"album_cover_path", L"AppData\\cache\\cover\\");
...
void CPlayer::LoadConfig()
{
    ...
    m_volume = ini.GetInt(L"config", L"volume", 100);
    ...
    theApp.m_app_setting_data.album_cover_path = L"AppData\\cache\\cover\\";
...
void CPlayer::SearchAlbumCover()
{
    ...
    //{
    DeleteCacheFile(ALBUM_COVER_NAME);
    DeleteCacheFile(ALBUM_COVER_TEMP_NAME);
    m_album_cover.Destroy();
    SongInfo song_info{ CSongDataManager::GetInstance().GetSongInfo3(GetCurrentSongInfo()) };
    ...
    if ((!theApp.m_app_setting_data.use_out_image || theApp.m_app_setting_data.use_inner_image_first) && !IsOsuFile() && !always_use_external_album_cover)
    {
        ...
        string album_cover_data{ audio_tag.GetAlbumCoverData(m_album_cover_type) };
        if (LoadImageFromMemory(album_cover_data, m_album_cover))
        {
            m_album_cover_path.clear();
            AlbumCoverResize();
            m_controls.loadThumbnail(reinterpret_cast<const BYTE*>(album_cover_data.data()), album_cover_data.size());
        }
    }
...
void CPlayer::AlbumCoverResize()
{
    ...
    if (!m_album_cover.IsNull() && theApp.m_nc_setting_data.max_album_cover_size > 0)
    {
        ...
        {
            CCommon::SizeZoom(image_size, theApp.m_nc_setting_data.max_album_cover_size);
            CImage img_temp;
            CDrawCommon::ImageResize(m_album_cover, img_temp, image_size);
            m_album_cover.Destroy();
            m_album_cover.Attach(img_temp.Detach());
