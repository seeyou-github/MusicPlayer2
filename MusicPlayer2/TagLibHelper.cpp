...
#define STR_ID3V2_USER_LYRIC_TAG "LYRICS"
...
static ByteVector AlbumCoverDataToByteVector(const string& album_cover_data)
{
    if (album_cover_data.empty())
        return ByteVector();
    return ByteVector(album_cover_data.data(), static_cast<unsigned int>(album_cover_data.size()));
}

static void WriteId3v2AlbumCover(ID3v2::Tag* id3v2tag, const ByteVector& pic_data, const std::wstring& ext);
static void WriteApeTagAlbumCover(APE::Tag* tag, const ByteVector& pic_data, const std::wstring& ext, bool remove_exist);
static void WriteXiphCommentAlbumCover(Ogg::XiphComment* tag, const ByteVector& pic_data, const std::wstring& ext, bool remove_exist);
...
static void WriteId3v2AlbumCover(ID3v2::Tag* id3v2tag, const std::wstring& album_cover_path)
{
    if (id3v2tag != nullptr)
    {
        ...
        WriteId3v2AlbumCover(id3v2tag, pic_data, ext);
    }
}

static void WriteId3v2AlbumCover(ID3v2::Tag* id3v2tag, const ByteVector& pic_data, const std::wstring& ext)
{
    if (id3v2tag != nullptr && !pic_data.isEmpty())
    {
        //向音频文件写入图片文件
...
static void WriteApeTagAlbumCover(APE::Tag* tag, const std::wstring& album_cover_path, bool remove_exist)
{
    ...
    if (!album_cover_path.empty())
    {
        ...
        WriteApeTagAlbumCover(tag, pic_data, file_type, false);
    }
}
...
static void WriteXiphCommentAlbumCover(Ogg::XiphComment* tag, const std::wstring& album_cover_path, bool remove_exist)
{
    ...
    if (!album_cover_path.empty())
    {
        ...
        FileToByteVector(pic_data, album_cover_path);
        std::wstring ext = CFilePathHelper(album_cover_path).GetFileExtension();
        WriteXiphCommentAlbumCover(tag, pic_data, ext, false);
    }
}
...
static std::wstring GetId3v2Lyric(ID3v2::Tag* id3v2)
{
    ...
    if (id3v2 != nullptr)
    {
        ...
        if (lyrics.empty())
        {
            auto user_lyric_frame = ID3v2::UserTextIdentificationFrame::find(id3v2, STR_ID3V2_USER_LYRIC_TAG);
            if (user_lyric_frame != nullptr)
            {
                auto description = user_lyric_frame->description();
                auto fields = user_lyric_frame->fieldList();
                for (const auto& field : fields)
                {
                    if (!field.isEmpty() && !(field == description))
                    {
                        lyrics = field.toWString();
                        break;
                    }
                }
            }
...
bool CTagLibHelper::WriteMp3AlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    MPEG::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    {
        auto id3v2tag = file.ID3v2Tag(true);
        WriteId3v2AlbumCover(id3v2tag, AlbumCoverDataToByteVector(album_cover_data), ext);
    }
    int tags = MPEG::File::ID3v2;
    if (file.hasAPETag())
        tags |= MPEG::File::APE;
    return file.save(tags, File::StripOthers, GetWriteId3v2Version());
}
...
bool CTagLibHelper::WriteFlacAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    FLAC::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    if (!album_cover_data.empty())
    {
        FLAC::Picture* newpic = new FLAC::Picture();
        newpic->setType(FLAC::Picture::FrontCover);
        newpic->setData(AlbumCoverDataToByteVector(album_cover_data));
        newpic->setMimeType(L"image/" + ext);
        file.addPicture(newpic);
    }
    return file.save();
...
bool CTagLibHelper::WriteM4aAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    MP4::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    if (remove_exist && tag->contains(STR_MP4_COVER_TAG))
        tag->removeItem(STR_MP4_COVER_TAG);
    ...
    if (!album_cover_data.empty())
    {
        ...
        if (ext == L"jpg" || ext == L"jpeg")
            ...
            format = MP4::CoverArt::Format::BMP;
        MP4::CoverArt cover_item(format, AlbumCoverDataToByteVector(album_cover_data));
...
bool CTagLibHelper::WriteAsfAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    ASF::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    if (remove_exist && tag->contains(STR_ASF_COVER_TAG))
        tag->removeItem(STR_ASF_COVER_TAG);

    if (!album_cover_data.empty())
    {
        ASF::Picture picture;
        picture.setMimeType(L"image/" + ext);
        picture.setType(ASF::Picture::FrontCover);
        picture.setPicture(AlbumCoverDataToByteVector(album_cover_data));
        tag->setAttribute(STR_ASF_COVER_TAG, picture);
    }
    return file.save();
}
...
bool CTagLibHelper::WriteWavAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    RIFF::WAV::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    if (!album_cover_data.empty())
        WriteId3v2AlbumCover(id3v2tag, AlbumCoverDataToByteVector(album_cover_data), ext);
    return file.save(RIFF::WAV::File::AllTags, File::StripOthers, GetWriteId3v2Version());
}
...
bool CTagLibHelper::WriteTtaAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    TrueAudio::File file(file_path.c_str());
    if (!file.isValid())
        return false;
...
bool CTagLibHelper::WriteApeAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    APE::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    auto tag = file.APETag(true);
    WriteApeTagAlbumCover(tag, AlbumCoverDataToByteVector(album_cover_data), ext, remove_exist);
    return file.save();
}
...
bool CTagLibHelper::WriteOggAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    Ogg::Vorbis::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    if (tag != nullptr)
    {
        WriteXiphCommentAlbumCover(tag, AlbumCoverDataToByteVector(album_cover_data), ext, remove_exist);
        return file.save();
    }
    return false;
}
...
bool CTagLibHelper::WriteOpusAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    Ogg::Opus::File file(file_path.c_str());
    if (!file.isValid())
        return false;
...
bool CTagLibHelper::WriteSpxAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    Ogg::Speex::File file(file_path.c_str());
    if (!file.isValid())
        return false;
...
bool CTagLibHelper::WriteAiffAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    RIFF::AIFF::File file(file_path.c_str());
    if (!file.isValid())
        return false;
    ...
    if (!album_cover_data.empty())
        WriteId3v2AlbumCover(id3v2tag, AlbumCoverDataToByteVector(album_cover_data), ext);
    return file.save(GetWriteId3v2Version());
}
...
bool CTagLibHelper::WriteMpcAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    MPC::File file(file_path.c_str());
    if (!file.isValid())
        return false;
...
bool CTagLibHelper::WriteWavePackAlbumCover(const std::wstring& file_path, const string& album_cover_data, const wstring& ext, bool remove_exist)
{
    WavPack::File file(file_path.c_str());
    if (!file.isValid())
        return false;
