...
void UiElement::AbstractListElement::DrawScrollArea()
{
    ...
    else
    {
        ...
        //设置字体
        UiFontGuard set_font(ui, EffectiveFontSize());
            ...
            if (!(rect_item & rect).IsRectEmpty())
            {
                ...
                if (IsHighlightRow(i))
                {
                        ...
                        && CPlayer::GetInstance().GetPlayingState2() != PS_STOPED)
                    {
                        ...
                        int spectrum_width{ (std::min)(ItemHeight(), ui->DPI((std::max)(24, 24 + (EffectiveFontSize() - 20) * 2))) };
                        const int index_width{ GetColumnCount() > 0 ? GetColumnWidth(0, rect_item.Width()) : 0 };
                        if (index_width > 0)
                            spectrum_width = (std::min)(spectrum_width, (std::max)(0, index_width - ui->DPI(12)));
                        rect_mini_spectrum.right = rect_mini_spectrum.left + spectrum_width;
                ...
                {
                    int icon_width{ (std::min)(ItemHeight(), ui->DPI((std::max)(20, 20 + (EffectiveFontSize() - 20) * 2))) };
                    CRect rect_icon{ rect_item };
                    rect_icon.left = col_x;
                    rect_icon.right = rect_icon.left + icon_width;
                    col_x = rect_icon.right;
                    rect_icon.MoveToX(rect_icon.left + indent_space);
                    ui->DrawUiIcon(rect_icon, cur_icon, IconMgr::IconStyle::IS_Auto, icon_width > ui->DPI(22) ? IconMgr::IconSize::IS_DPI_20 : IconMgr::IconSize::IS_DPI_16);
                }
                ...
                for (int j{}; j < GetColumnCount(); j++)
                {
                    ...
                    //绘制鼠标指向时的按钮
                    if (ShowHoverButtons() && GetHoverButtonCount(i) > 0 && GetHoverButtonColumn() == j && rect_cell.PtInRect(mouse_pos))
                    {
                        const int btn_size{ (std::min)(ItemHeight(), ui->DPI((std::max)(24, 24 + (EffectiveFontSize() - 20) * 2))) };
...
bool UiElement::AbstractListElement::LButtonUp(CPoint point)
{
    ...
    if (pressed_tmp && rect.PtInRect(point))
    {
        ...
        //设置按钮的按下状态
        for (int i{}; ShowHoverButtons() && i < GetHoverButtonCount(row); i++)
        {
...
bool UiElement::AbstractListElement::LButtonDown(CPoint point)
{
    ...
    if (rect.PtInRect(point) && !scrollbar_rect.PtInRect(point))
    {
        ...
        //设置按钮的按下状态
        for (int i{}; ShowHoverButtons() && i < GetHoverButtonCount(clicked_index); i++)
        {
            auto& btn{ GetHoverButtonState(i) };
...
bool UiElement::AbstractListElement::MouseMove(CPoint point)
{
    ...
    if (ShowHoverButtons() && GetHoverButtonCount(row) > 0)
    {
...
{
    return ui->DPI(EffectiveItemHeight());
}

bool UiElement::AbstractListElement::IsSongList() const
{
    return dynamic_cast<const UiElement::Playlist*>(this) != nullptr
        || dynamic_cast<const UiElement::AbstractTracksList*>(this) != nullptr
        || dynamic_cast<const UiElement::MyFavouriteList*>(this) != nullptr;
}

bool UiElement::AbstractListElement::ShowHoverButtons() const
{
    return true;
}

int UiElement::AbstractListElement::EffectiveFontSize() const
{
    if (IsSongList())
        return theApp.m_app_setting_data.song_list_font_size;
    return font_size;
}

int UiElement::AbstractListElement::EffectiveItemHeight() const
{
    if (IsSongList())
    {
        int cached_item_height{ GetSongListCachedItemHeight() };
        if (cached_item_height > 0)
            return cached_item_height;
        return CalculateSongListItemHeight(theApp.m_app_setting_data.song_list_font_size);
    }
    return item_height;
}

COLORREF UiElement::AbstractListElement::ListTextColor() const
{
    if (IsSongList())
...
void UiElement::AbstractListElement::DrawHoverButton(int index, int row)
{
    ...
    ui->DrawUIButton(rc_button, GetHoverButtonState(index), GetHoverButtonIcon(index, row), ItemHeight() > ui->DPI(32));
}
...
{
    ui->DrawUiIcon(rc_button, GetUnHoverIcon(index, row), IconMgr::IconStyle::IS_Auto, ItemHeight() > ui->DPI(32) ? IconMgr::IconSize::IS_DPI_20 : IconMgr::IconSize::IS_DPI_16);
}
