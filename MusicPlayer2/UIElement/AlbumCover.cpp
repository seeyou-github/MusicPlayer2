...
{
    CalculateRect();
    int draw_radius{ radius >= 0 ? ui->DPI(radius) : radius };
    if (show_info)
        ui->DrawAlbumCoverWithInfo(rect, draw_radius);
    else
        ui->DrawAlbumCover(rect, draw_radius);
    Element::Draw();
}
...
void UiElement::AlbumCover::FromXmlNode(tinyxml2::XMLElement* xml_node)
{
    ...
    CTinyXml2Helper::GetElementAttributeInt(xml_node, "radius", radius);
    CTinyXml2Helper::GetElementAttributeInt(xml_node, "radus", radius);
}
