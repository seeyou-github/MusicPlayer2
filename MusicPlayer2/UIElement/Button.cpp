...
void UiElement::Button::Draw()
{
    ...
    if (!empty_btn)
    {
        ...
        case CPlayerUIBase::BTN_LRYIC:
        {
            ...
            static const COLORREF enabled_color = RGB(0, 190, 90);
            ui->DrawTextButton(rect, m_btn, btn_str.c_str(), theApp.m_lyric_setting_data.show_desktop_lyric, &enabled_color, false, icon_size);
        }
        break;
        ...
        default:
        {
            ...
            if (show_text)
                text = GetDisplayText();
            ui->DrawUIButton(rect, m_btn, GetBtnIconType(), big_icon, text, font_size, false, align, btn_background, icon_size);
        }
        break;
        }
    }
...
void UiElement::Button::SetText(const std::wstring& str_text)
{
    text = str_text;
...
void UiElement::Button::FromXmlNode(tinyxml2::XMLElement* xml_node)
{
    ...
    CTinyXml2Helper::GetElementAttributeInt(xml_node, "icon_size", icon_size);
