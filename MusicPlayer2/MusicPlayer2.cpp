...
BOOL CMusicPlayerApp::InitInstance()
{
    ...
    SetDllDirectoryW((m_module_dir + L"lib\\").c_str());
...
#endif // _DEBUG
    m_appdata_dir = m_module_dir + L"AppData\\";
    m_config_dir = m_appdata_dir + L"config\\";
    m_data_dir = m_appdata_dir + L"data\\";
    m_cache_dir = m_appdata_dir + L"cache\\";
    CCommon::CreateDir(m_appdata_dir);
    CCommon::CreateDir(m_config_dir);
    CCommon::CreateDir(m_data_dir);
    CCommon::CreateDir(m_cache_dir);
    CCommon::CreateDir(m_cache_dir + L"lyrices\\");
    CCommon::CreateDir(m_cache_dir + L"cover\\");

    m_config_path = m_config_dir + L"config.ini";
    m_song_data_path = m_data_dir + L"song_data.dat";
    m_recent_list_dat_path = m_data_dir + L"recent_list.dat";
    // 以下三项不再使用，但路径留作旧版兼容，新程序不再创建这三个文件
    m_recent_path_dat_path = m_data_dir + L"recent_path.dat";
    m_recent_playlist_data_path = m_data_dir + L"playlist\\recent_playlist.dat";
    m_recent_medialib_playlist_path = m_data_dir + L"recent_medialib_item.dat";

    m_desktop_path = CCommon::GetDesktopPath();
    m_lastfm_path = m_data_dir + L"lastfm.dat";
    m_ui_data_path = m_data_dir + L"user_ui.dat";
    m_playlist_dir = m_data_dir + L"playlist\\";
...
{
    m_general_setting_data.portable_mode = true;
    m_module_dir_writable = true;
}
...
void CMusicPlayerApp::LoadConfig()
{
    ...
    m_general_setting_data.check_update_when_start = ini.GetBool(L"general", L"check_update_when_start", false);
    m_general_setting_data.update_source = ini.GetInt(L"general", L"update_source", 0);
