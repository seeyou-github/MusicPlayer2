...
void CPlayerUIBase::DrawInfo(bool reset, CRgn* draw_rgn)
{
    ...
    //双缓冲绘图
    {
        ...
        if (draw_status_bar)
        {
            ...
            rc_status_bar.top = rc_status_bar.bottom - DPI(20);
            if (theApp.m_app_setting_data.always_show_statusbar)
                draw_rect.bottom = rc_status_bar.top;
        }
        //标题栏
        if (IsDrawTitleBar())
        {
...
void CPlayerUIBase::DrawBackground()
{
    ...
    if (theApp.m_app_setting_data.enable_background && m_ui_data.enable_background)
    {
        ...
        else
        {
            ...
            if (!m_ui_data.default_background.IsNull())
...
void CPlayerUIBase::DrawUIButton(const CRect& rect, UIButton& btn, IconMgr::IconType icon_type, bool big_icon, const std::wstring& text, int font_size, bool checked, Alignment align, bool btn_background, int icon_size)
{
    btn.rect = rect;
    ...
    {
        const int icon_size_px = (icon_size > 0 ? DPI(icon_size) : DPI(big_icon ? 20 : 16));
        //计算图标矩形区域
        if (!text.empty())
        {
            //如果要显示文本，则图标和文本一起居中显示
            int content_width = icon_size_px + DPI(16) + m_draw.GetTextExtent(text.c_str()).cx;    //图标和文本的宽度（左侧4px+图标宽度+图标和文本间距8px+右侧4px+文本宽度）
            if (align == Alignment::CENTER)
                rect_icon.left += (rc_tmp.Width() - content_width) / 2;
            else if (align == Alignment::RIGHT)
                rect_icon.left += (rc_tmp.Width() - content_width);
            else
                rect_icon.left += (rc_tmp.Height() - icon_size_px - DPI(8)) / 2;
            rect_icon.right = rect_icon.left + icon_size_px + DPI(8);
        }
        ...
        if (icon_size > 0)
            ...
            DrawUiIcon(rect_icon, icon_type, icon_style, big_icon ? IconMgr::IconSize::IS_DPI_20 : IconMgr::IconSize::IS_DPI_16);
    }
...
void CPlayerUIBase::DrawTextButton(CRect rect, BtnKey btn_type, LPCTSTR text, bool checked, const COLORREF* checked_text_color, bool checked_background, int font_size)
{
    auto& btn = m_buttons[btn_type];
    DrawTextButton(rect, btn, text, checked, checked_text_color, checked_background, font_size);
}

void CPlayerUIBase::DrawTextButton(CRect rect, UIButton& btn, LPCTSTR text, bool checked, const COLORREF* checked_text_color, bool checked_background, int font_size)
{
    std::unique_ptr<UiFontGuard> font_guard;
    if (font_size > 0)
        font_guard = std::make_unique<UiFontGuard>(this, font_size);
    ...
    if (btn.enable)
    {
        ...
        if (!is_close_btn && IsDrawBackgroundAlpha())
            ...
            alpha = 255;
        if (btn.pressed || btn.hover || (checked && checked_background))
        {
        ...
        else if (checked && checked_text_color != nullptr)
            front_color = *checked_text_color;
...
bool CPlayerUIBase::IsDrawBackgroundAlpha() const
{
    ...
    if (theApp.m_app_setting_data.album_cover_as_background)
        return CPlayer::GetInstance().AlbumCoverExist() || !m_ui_data.default_background.IsNull();
    return !m_ui_data.default_background.IsNull();
}
...
CRect CPlayerUIBase::DrawProgressBar(CRect rect, bool play_time_both_side, int bar_height, const COLORREF* progress_back_color, const COLORREF* progress_color, const COLORREF* time_color)
{
    //绘制播放时间
    bool draw_progress_time{ rect.Width() > DPI(110) };
    CRect progress_rect{ rect };
    COLORREF text_color{ time_color != nullptr ? *time_color : m_colors.color_text };
    if (draw_progress_time)
    {
        ...
        if (play_time_both_side)
        {
            ...
            m_draw.DrawWindowText(rc_time_left, str_cur_time.c_str(), text_color);
            m_draw.DrawWindowText(rc_time_right, str_song_length.c_str(), text_color);
        ...
        else
        {
            ...
            rc_time.top -= DPI(1);
            m_draw.DrawWindowText(rc_time, strTime.c_str(), text_color);
            progress_rect.right = rc_time.left - Margin();
        }
    }

    //绘制进度条
    return DrawProgess(progress_rect, bar_height, progress_back_color, progress_color);
}

CRect CPlayerUIBase::DrawProgess(CRect rect, int bar_height, const COLORREF* progress_back_color, const COLORREF* progress_color)
{
    //进度条的高度
    int progress_height = (bar_height > 0 ? min(bar_height, rect.Height()) : min(DPI(4), rect.Height()));
    rect.top = rect.top + (rect.Height() - progress_height) / 2;
    rect.bottom = rect.top + progress_height;

    COLORREF back_color{ progress_back_color != nullptr ? *progress_back_color : m_colors.color_progress_back };
    COLORREF bar_color{ progress_color != nullptr ? *progress_color : m_colors.color_spectrum };
    if (IsDrawBackgroundAlpha())
        m_draw.FillAlphaRect(rect, back_color, ALPHA_CHG(theApp.m_app_setting_data.background_transparency) * 2 / 3);
    else
        m_draw.FillRect(rect, back_color);
    ...
    if (played_rect.right > played_rect.left)
        m_draw.FillRect(played_rect, bar_color);
    //绘制进度条最右侧一像素
    ...
    m_draw.FillAlphaRect(played_rect, bar_color, alpha);
...
CRect CPlayerUIBase::GetClientDrawRect() const
{
    ...
    if (theApp.m_app_setting_data.always_show_statusbar && IsDrawStatusBar())
    {
...
void CPlayerUIBase::DrawAlbumCover(CRect rect, int radius)
{
    DrawAreaGuard guard(&m_draw, rect);
    if (theApp.m_app_setting_data.show_album_cover && CPlayer::GetInstance().AlbumCoverExist())
    {
        if (radius >= 0)
        {
            m_draw.DrawRoundImage(CPlayer::GetInstance().GetAlbumCover(), radius, rect.TopLeft(), rect.Size(), theApp.m_app_setting_data.album_cover_fit);
        }
        else if (theApp.m_app_setting_data.draw_album_high_quality)
        {
            if (theApp.m_app_setting_data.button_round_corners)
...
void CPlayerUIBase::DrawAlbumCoverWithInfo(CRect rect, int radius)
{
    const int info_height{ DPI(60) };   //歌曲信息区域的高度
    if (IsDrawBackgroundAlpha())
    {
        DrawAlbumCover(rect, radius);
    }
    else        //如果不绘制透明背景，则将专辑封面显示在歌曲信息区域的上方
    {
        CRect rect_temp{ rect };
        rect_temp.bottom -= info_height;
        DrawAlbumCover(rect_temp, radius);
    }
...
void CPlayerUIBase::DrawLyrics(CRect rect, CFont* lyric_font, CFont* lyric_tr_font, bool with_background, bool show_song_info,
    const COLORREF* text_color, const COLORREF* playing_text_color)
{
    ...
    m_draw.SetLyricColors(text_color, playing_text_color);
    m_draw.DrawLryicCommon(rect, theApp.m_lyric_setting_data.lyric_align, show_song_info);
    m_draw.SetLyricColors(nullptr, nullptr);
}
...
void CPlayerUIBase::DrawUiIcon(const CRect& rect, IconMgr::IconType icon_type, IconMgr::IconStyle icon_style, int icon_size)
{
    if (icon_style == IconMgr::IconStyle::IS_Auto)
        icon_style = theApp.m_app_setting_data.dark_mode ? IconMgr::IconStyle::IS_OutlinedLight : IconMgr::IconStyle::IS_OutlinedDark;

    int icon_size_px = DPI(icon_size);
    if (IsDrawLargeIcon())
        icon_size_px = static_cast<int>(icon_size_px * CONSTVAL::FULL_SCREEN_ZOOM_FACTOR);

    IconMgr::IconSize resource_icon_size{ IconMgr::IconSize::IS_DPI_16 };
    if (icon_size >= 24)
        resource_icon_size = IconMgr::IconSize::IS_DPI_32;
    else if (icon_size > 16)
        resource_icon_size = IconMgr::IconSize::IS_DPI_20;

    CSize size_icon{ icon_size_px, icon_size_px };
    CPoint pos_icon{ rect.left + (rect.Width() - size_icon.cx) / 2 , rect.top + (rect.Height() - size_icon.cy) / 2 };
    HICON hIcon = theApp.m_icon_mgr.GetHICON(icon_type, icon_style, resource_icon_size);
    m_draw.DrawIcon(hIcon, pos_icon, size_icon);
}
