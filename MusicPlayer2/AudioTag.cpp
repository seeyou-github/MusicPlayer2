...
bool CAudioTag::GetAudioTag()
{
    ...
    else
    {
        ...
        case AU_MPC:
            ...
            break;
        case AU_OPUS:
            if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
                CTagLibHelper::GetMatroskaTagInfo(m_song_info);
            else
...
void CAudioTag::GetAudioTagPropertyMap(std::map<wstring, wstring>& property_map)
{
    ...
    else
    {
        ...
        case AU_OPUS:
            if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
                CTagLibHelper::GetMatroskaPropertyMap(m_song_info.file_path, property_map);
            else
...
string CAudioTag::GetAlbumCoverData(int& image_type)
{
    image_type = -1;
    ...
    case AU_OPUS:
        if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
            image_contents = CTagLibHelper::GetMatroskaAlbumCover(m_song_info.file_path, image_type);
        else
...
{
    string image_contents{ GetAlbumCoverData(image_type) };
...
wstring CAudioTag::GetAudioLyric()
{
    ...
    case AU_OPUS:
        if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
            return CTagLibHelper::GetMatroskaLyric(m_song_info.file_path);
        break;
    default:
        break;
    }
...
bool CAudioTag::WriteAudioLyric(const wstring& lyric_contents)
{
    ...
    case AU_OPUS:
        if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
            return CTagLibHelper::WriteMatroskaLyric(m_song_info.file_path, lyric_contents);
        break;
    default:
        break;
    }
...
bool CAudioTag::WriteAudioTag()
{
    ...
    else
    {
        ...
        case AU_OPUS:
            if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
                return CTagLibHelper::WriteMatroskaTag(m_song_info);
...
bool CAudioTag::WriteAlbumCover(const wstring& album_cover_path)
{
    ...
    case AU_OPUS:
        if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
            return CTagLibHelper::WriteMatroskaAlbumCover(m_song_info.file_path, album_cover_path);
...
bool CAudioTag::WriteAlbumCover(const string& album_cover_data, const wstring& ext)
{
    switch (m_type)
    {
    case AU_MP3:
        return CTagLibHelper::WriteMp3AlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_WMA_ASF:
        return CTagLibHelper::WriteAsfAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_OGG:
        return CTagLibHelper::WriteOggAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_MP4:
        return CTagLibHelper::WriteM4aAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_APE:
        return CTagLibHelper::WriteApeAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_AIFF:
        return CTagLibHelper::WriteAiffAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_FLAC:
        return CTagLibHelper::WriteFlacAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_WAV:
        return CTagLibHelper::WriteWavAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_MPC:
        return CTagLibHelper::WriteMpcAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_OPUS:
        if (CFilePathHelper(m_song_info.file_path).GetFileExtension() == L"webm")
            return CTagLibHelper::WriteMatroskaAlbumCover(m_song_info.file_path, album_cover_data, ext);
        return CTagLibHelper::WriteOpusAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_WV:
        return CTagLibHelper::WriteWavePackAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_SPX:
        return CTagLibHelper::WriteSpxAlbumCover(m_song_info.file_path, album_cover_data, ext);
    case AU_TTA:
        return CTagLibHelper::WriteTtaAlbumCover(m_song_info.file_path, album_cover_data, ext);
    default:
        break;
    }
    return false;
...
bool CAudioTag::IsFileTypeCoverWriteSupport(const wstring& ext)
{
    ...
    return type == AU_MP3 || type == AU_FLAC || type == AU_MP4 || type == AU_WMA_ASF || type == AU_WAV || type == AU_APE
        || type == AU_OGG || _ext == L"webm" || _ext == L"opus" || type == AU_SPX || type == AU_AIFF || type == AU_MPC || type == AU_WV
        || type == AU_TTA;
}
...
bool CAudioTag::IsFileTypeLyricWriteSupport(const wstring& ext)
{
    ...
    CCommon::StringTransform(_ext, false);
    AudioType type = CAudioCommon::GetAudioTypeByFileExtension(_ext);
    return type == AU_MP3 || type == AU_FLAC || type == AU_MP4 || type == AU_WMA_ASF || type == AU_WAV || _ext == L"webm";
}
