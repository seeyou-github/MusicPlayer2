...
void CTabCtrlEx::AddWindow(CWnd* pWnd, LPCTSTR lable_text, IconMgr::IconType icon_type)
{
    ...
    AddWindowPlaceholder(lable_text, icon_type);
    SetTabWindow(static_cast<int>(m_tab_list.size()) - 1, pWnd);
}

void CTabCtrlEx::AddWindowPlaceholder(LPCTSTR lable_text, IconMgr::IconType icon_type)
{
    InsertItem(static_cast<int>(m_tab_list.size()), lable_text, static_cast<int>(m_tab_list.size()));
    m_tab_list.push_back(nullptr);
    if (icon_type != IconMgr::IconType::IT_NO_ICON)
        m_icon_list.push_back(icon_type);
}

void CTabCtrlEx::SetTabWindow(int index, CWnd* pWnd)
{
    if (index < 0 || index >= static_cast<int>(m_tab_list.size()) || pWnd == nullptr || pWnd->GetSafeHwnd() == NULL)
        return;

	pWnd->SetParent(this);
	pWnd->MoveWindow(m_tab_rect);
    pWnd->ShowWindow(SW_HIDE);
    m_tab_list[index] = pWnd;
}
...
void CTabCtrlEx::SetCurTab(int index)
{
    ...
    if (m_tab_list[index] == nullptr)
    {
        CWnd* pParent = GetParent();
        if (pParent != nullptr)
            pParent->SendMessage(WM_TAB_WINDOW_REQUIRED, index, reinterpret_cast<LPARAM>(this));
    }
    if (m_tab_list[index] == nullptr)
        return;
...
CWnd* CTabCtrlEx::GetCurrentTab()
{
    ...
    if (cur_tab_index >= 0 && cur_tab_index < static_cast<int>(m_tab_list.size()) && m_tab_list[cur_tab_index] != nullptr)
    {
