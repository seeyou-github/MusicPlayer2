...
{
    if (!theApp.m_cache_dir.empty())
    {
        CCommon::CreateDir(theApp.m_cache_dir);
        return theApp.m_cache_dir;
    }

    wstring app_data_path = CCommon::GetExePath() + L"AppData\\";
    CCommon::CreateDir(app_data_path);
    wstring result = app_data_path + L"cache\\";
    CCommon::CreateDir(result);
    return result;
}
