...
void CSettingsPanelMediaLib::OnDeleteMediaLibFolderClicked()
{
    ...
    if (index >= 0 && index < static_cast<int>(m_data.media_folders.size()))
    {
        ...
        if (theApp.m_pMainWnd->MessageBox(str_info.c_str(), NULL, MB_ICONQUESTION | MB_YESNO) == IDYES)
        {
            ...
            for (int i = static_cast<int>(m_data.folder_tab_paths.size()) - 1; i >= 0; --i)
            {
                std::wstring tab_path{ m_data.folder_tab_paths[i] };
                if (!tab_path.empty() && tab_path.back() != L'\\' && tab_path.back() != L'/')
                    tab_path.push_back(L'\\');
                std::wstring selected_folder{ str_selected_folder };
                if (!selected_folder.empty() && selected_folder.back() != L'\\' && selected_folder.back() != L'/')
                    selected_folder.push_back(L'\\');
                if (CCommon::StringCompareNoCase(tab_path, selected_folder))
                {
                    m_data.folder_tab_paths.erase(m_data.folder_tab_paths.begin() + i);
                    if (i < static_cast<int>(m_data.folder_tab_names.size()))
                        m_data.folder_tab_names.erase(m_data.folder_tab_names.begin() + i);
                }
            }
